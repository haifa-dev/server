...
exports.getDevProfiles = async (req, res) => {
  ...
  if (offset && limit) devProfiles = await DevProfile.findAndCountAll({ offset, limit, raw: true });
  else if (offset) devProfiles = await DevProfile.findAndCountAll({ offset, raw: true });
  else if (limit) devProfiles = await DevProfile.findAndCountAll({ limit, raw: true });
...
exports.createDevProfile = async (req, res) => {
  ...
  devProfile = await DevProfile.create(_.pick(req.body, ['email', 'name', 'image', 'bio']));
